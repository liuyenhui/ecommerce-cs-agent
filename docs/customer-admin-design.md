...
- 客户 Admin 后台不做 Notion 化，应继续采用 IBM / Carbon 式密集、可扫描、低阴影、1px hairline、表格 / 队列 / 配置表单优先的企业操作台规则。
...
客户后台 UI 的源文档是本文。系统后台的具体布局和内部平台治理规则由 [System Admin Design](system-admin-design.md) 与 [System Admin UI Prototype](system-admin-ui-prototype.html) 承载；本文只描述客户后台应如何复用企业后台视觉规则，避免在多个文档重复维护同一套设计细节。

第一版客户后台实现应保持以下基准：

- 站点和入口：`admin.ecommerce-cs-agent-dev.fcihome.com` 承载公开宣传页 `/`、客户登录页 `/login` 和受保护客户后台 `/admin`。
- 路由守卫：客户后台 shell 只调用 `/v1/admin/auth/me` 校验客户 Admin session，不探测、不复用 `/v1/system-admin/auth/me`。
- Shell 结构：登录后的客户后台使用黑色顶栏、浅灰左侧分组导航、主内容区和右侧客户上下文栏；视觉密度、表格、筛选、状态徽标、抽屉、模态和 Toast 与系统后台保持同一 Carbon 式控制台基线。
- 信息架构：客户后台导航只包含客户运营能力，即首页概览、商品资料、资料上传、价格快照、知识审核、规则配置、动作能力、审计与追踪。
- 禁止入口：客户后台不得展示“系统后台”按钮、tab、workspace switch、侧栏入口或系统后台用户信息。
- 上下文栏：右侧只展示当前组织、当前店铺、资料缺口、待审核知识、最近变更等客户侧上下文，不展示跨租户运行摘要或平台告警。
- 动作规则：商品、知识、规则、动作能力和高风险配置变更必须要求明确原因，并进入客户审计。
- 响应式规则：移动端隐藏右侧上下文栏；表格用关键字段列表或 `data-label` 形式降级，不允许横向溢出、文字重叠或主操作不可点击。
