...
| `product_master` / `product_profile` | `id`；`product_master_ref` 或 `(store_id, external_product_id)` | `store_id` | 通用商品资料、`title`、`category`、`status`、`description`、`metadata JSONB`、`updated_at` |
| `listing` / `store_product` | `id`；`(store_id, platform, listing_ref)` | `store_id`、`platform_account_id`、`product_profile_id` | 某平台、某店铺、某销售链接或 SKU 的售卖实例、标题、状态、平台商品引用 |
| `product_sku_profile` | `id` | `product_profile_id`、`listing_id` 可选 | `external_sku_id`、`sku_code`、`spec JSONB`、`attributes JSONB`、`status` |
...
external_store_id / platform_account_ref / listing_ref
