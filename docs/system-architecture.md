...
| Auth Middleware | 识别接入方、解析外部店铺 / listing 引用并校验租户、店铺权限 | API Key 或 Bearer Token |
...
| 表 | 主键 / 唯一键 | 关键外键 | 关键字段 |
| --- | --- | --- | --- |
| `organization` / 内部 `tenant` | `id` | - | `name`、`status`、`external_ref`、`metadata JSONB`；外部接入不要求传 `organization_id` |
| `store` | `id` | `organization_id` / `tenant_id` | `name`、`platform`、`external_store_id`、`settings JSONB` |
| `platform_account` | `id` | `store_id` | `platform`、`external_account_id`、`auth_ref`、`capabilities JSONB` |
| `product_master` | `id`；`tenant_id` | `tenant_id` | `title`、`category`、`brand`、`description`、`metadata JSONB`、`status`、`updated_at` |
| `product_sku` | `id` | `product_master_id` | `sku_code`、`spec JSONB`、`attributes JSONB`、`status` |
| `listing` / `store_product` | `id`；`(store_id, platform, external_listing_id)` | `store_id`、`platform_account_id`、`product_master_id`、`product_sku_id` | `external_product_id`、`external_sku_id`、`listing_url`、`sales_status`、`metadata JSONB` |
| `product_asset` | `id` | `product_master_id`、`product_sku_id`、`listing_id` | `asset_type`、`file_ref`、`file_hash`、`version`、`review_status`、`allow_customer_quote` |
| `product_asset_markdown` | `id` | `product_asset_id` | `markdown_text`、`source_map JSONB`、`conversion_status`、`reviewer_id`、`reviewed_at` |
| `product_price_snapshot` | `id` | `listing_id`、`product_master_id`、`product_sku_id`、`store_id` | `source`、`current_price`、`campaign_price`、`effective_at`、`expires_at`、`external_updated_at`、`status` |
...
| `product_knowledge_candidate` | `id` | `product_asset_markdown_id`、`product_master_id`、`listing_id`、`store_id` | `candidate_text`、`section_ref`、`source_excerpt`、`qa_intent`、`coverage_flags JSONB`、`risk_flags JSONB`、`status` |
...
| `knowledge_eval_case` | `id` | `knowledge_entry_id`、`product_master_id`、`listing_id` | `question`、`expected_answer`、`paraphrases JSONB`、`source_refs JSONB`、`coverage_flags JSONB`、`review_status` |
| `rule_set` | `id` | `store_id` | `rule_type`、`priority`、`condition JSONB`、`action JSONB`、`enabled`、`version`、`effective_at` |

关键约束：

- 所有业务表带内部 `tenant_id` / `organization_id` 或可通过 `store_id` 回溯到租户，避免跨店铺串数据；外部接入 API 使用 `external_store_id`、`platform_account_ref`、`listing_ref` 映射到内部边界。
- 外部 API 不接收内部 `tenant_id`；外部系统只提交 `platform`、`external_store_id` / `platform_account_ref`、`external_product_id` / `external_sku_id` / `listing_ref` 等业务引用，服务端解析到内部租户、店铺、平台账号、销售实例和商品主数据。
- 外部幂等靠 `request_id`、`external_message_id`、`external_conversation_id` 唯一约束，不靠文本去重。
- 商品资料以 `product_master`、`product_sku`、`listing` / `store_product`、`product_asset` 管理；原始文件必须保留，Markdown 只是审稿稿件，不直接作为自动回复知识源。
- 检索优先级固定为：`listing/SKU` 专属知识 -> 店铺规则 -> 平台规则 -> `product_master` 通用知识 -> 租户通用知识。上下文不足时返回 `context_request`，不猜商品、价格、活动或售后规则。
...
graph_version
internal tenant_id / store_id / platform_account_id / listing_id
external_store_id / platform_account_ref / external_product_id / external_sku_id
...
sequenceDiagram
  ...
  Admin->>Asset: 上传说明书 / 照片 / 视频，维护商品主数据、SKU 和 listing
  Asset->>DB: 保存 product_master / product_sku / listing / product_asset
...
erDiagram
  ...
  STORE ||--o{ LISTING : sells
  PLATFORM_ACCOUNT ||--o{ LISTING : publishes
  PRODUCT_MASTER ||--o{ LISTING : maps
  PRODUCT_SKU ||--o{ LISTING : maps
  PRODUCT_MASTER ||--o{ PRODUCT_ASSET : uploads
  PRODUCT_SKU ||--o{ PRODUCT_ASSET : applies
  LISTING ||--o{ PRODUCT_ASSET : scopes
  PRODUCT_ASSET ||--o{ PRODUCT_ASSET_MARKDOWN : converts
  PRODUCT_ASSET_MARKDOWN ||--o{ PRODUCT_KNOWLEDGE_CANDIDATE : extracts
  LISTING ||--o{ PRODUCT_PRICE_SNAPSHOT : prices
...
- `external_store_id` / `platform_account_ref` / `listing_ref`：外部接入解析内部租户、店铺、平台账号和销售实例。
- `capability_manifest`：外部系统支持的 `action_type`、参数 schema、风险级别和回调地址。
- `action_request` / `action_result`：Agent 请求外部系统执行动作，以及外部系统回传执行结果。
- 内部 `tenant_id`、`store_id`、`platform_account_id`、`listing_id`、`platform`：多租户和销售实例隔离。
