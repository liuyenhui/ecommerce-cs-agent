...
| API image | `ghcr.io/liuyenhui/ecommerce-cs-agent-api:<tag>` |
| Admin image | `ghcr.io/liuyenhui/ecommerce-cs-agent-admin:<tag>` |
| 指定 tag 发布 | 推送分支 `codex/publish-<tag>`，或手动运行 `Publish Images` 并输入 `tag` |
| 发布前检查 | Python tests、Helm lint、Helm template |

发布完成后，用 namespace 内 `ghcr-auth` 创建临时 pull-check Pod，并设置 `imagePullPolicy: Always` 验证 GHCR 远端 tag 可拉取。
