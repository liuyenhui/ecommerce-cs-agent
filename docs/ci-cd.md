...
| Admin Web checks | `admin-web` 执行 `npm ci`、`npm test` 和 `npm run build`，阻断 TypeScript、Vite 构建、真实 API 调用代码、客户 / 系统后台路由守卫和拆站回归。 |
...
| 部署后验证 | 等待 K8s rollout 完成，执行 API health、Customer Admin health、System Admin health 和 quick live eval。 |
| 发布记录 | `scripts/run_dev_release_gate.py` 记录镜像 tag、Git SHA、GitOps commit、Flux / HelmRelease / Deployment 状态、migration 版本、API / Customer Admin / System Admin health 和 quick live eval 摘要，并由 `Deploy Dev GitOps` 上传 artifact。 |
