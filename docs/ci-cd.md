...
| Admin Web checks | `admin-web` 执行 `npm ci`、`npm test` 和 `npm run build`，阻断 TypeScript、Vite 构建或真实 API 调用代码的回归。 |
...
| Admin Web 构建门禁 | `Publish Images` verify job 在镜像构建前运行 `admin-web` 的 `npm ci`、`npm test` 和 `npm run build`，避免到 Docker build 阶段才发现前端类型或构建失败。 |
