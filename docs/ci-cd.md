...
| Helm lint / template / K8s 安全渲染检查 | 已在 PR checks 和 `Publish Images` verify job 中执行。 |
...
| K8s rollout 与 health / live eval | 已由 `.github/workflows/deploy-dev.yml` 的 `Verify Dev Release Gate` job 执行并上传 `dev-release-gate-<image_tag>` artifact；release gate 会先校验 runtime Secret key、Flux / Helm / image tag / rollout / migration，失败时采集诊断并停止，不再继续对旧版本跑 health / live eval。本地也可用 `scripts/run_dev_release_gate.py` 复跑。 |
...
`KUBECONFIG` 只用于发布后自动验收 job 读取 Flux / HelmRelease / Deployment 状态、触发 reconcile、校验 Kubernetes Secret key 名称并从 Kubernetes Secret 读取 `AGENT_API_TOKEN`；不用于 `kubectl set image`、`kubectl apply` 或绕过 GitOps 修改目标状态。默认部署路径仍通过 GitOps repo 的 tag / values 变更驱动 Flux。
...
| Admin Web checks | `admin-web` 执行 `npm ci`、`npm test` 和 `npm run build`，并对构建后的 Admin Nginx 镜像运行 `nginx -t`，阻断 TypeScript、Vite 构建、真实 API 调用代码、客户 / 系统后台路由守卫、拆站回归和 Nginx 配置回归。 |
| Helm lint/template | Helm chart、values、Secret 引用、Ingress、Service、Deployment、migration job 渲染检查。 |
| K8s security render check | 渲染 dev chart 后检查非 root、RuntimeDefault seccomp、只读根文件系统、drop ALL capabilities、资源 requests/limits、Deployment 探针和非 latest 镜像 tag。 |
...
| Admin Web 构建门禁 | `Publish Images` verify job 在镜像构建前运行 `admin-web` 的 `npm ci`、`npm test` 和 `npm run build`，对构建后的 Admin Nginx 镜像运行 `nginx -t`，并检查客户后台 host 不展示系统后台入口、系统后台 host 使用系统后台专用登录页和路由守卫。 |
| K8s 配置安全 | `scripts/check_k8s_security.py` 在 PR checks 和 `Publish Images` verify job 中阻断缺少安全上下文、资源限制、探针或使用 latest tag 的 chart。 |
| 部署后验证 | 先校验 runtime Secret contract、Flux / Helm / image tag / rollout / migration；全部通过后才执行 API health、Customer Admin health、System Admin health 和 quick live eval。 |
| 发布记录 | `scripts/run_dev_release_gate.py` 记录镜像 tag、Git SHA、GitOps commit、runtime Secret contract、Flux / HelmRelease / Deployment 状态、migration 版本、API / Customer Admin / System Admin health 和 quick live eval 摘要；如果前置部署检查失败，会记录 HelmRelease conditions、namespace events 和相关 Pod 日志摘要，并由 `Deploy Dev GitOps` 上传 artifact。 |
...
quick live eval 需要从 Secret 注入 `TARGET_BASE_URL` 和 `AGENT_API_TOKEN`，不能把 token 写入仓库或文档。`Deploy Dev GitOps` 会优先从 Kubernetes Secret `ecommerce-cs-agent-runtime` 读取 `AGENT_API_TOKEN` 并只传给 eval 子进程；报告和日志会脱敏。release gate 在 Helm reconcile 前只校验 runtime Secret 的 key 名称是否满足 contract，包括 AI CS 集成所需的 `OPEN_ERP_INTEGRATION_TOKEN` 和 `OPEN_ERP_BILLING_LEASE_SECRET`，不输出 Secret 值；缺 key 直接失败，不等待 Pod 进入 CrashLoop。红线 suite 一票否决，至少覆盖跨租户泄露、无依据报价、越权动作、绕过人工确认、高风险自动回复和 trace 缺失。

如果 HelmRelease 已处于失败、回滚或超时状态，release gate 会在新 GitOps commit 到来后执行一次受控的 `reconcile.fluxcd.io/resetAt` + `reconcile.fluxcd.io/requestedAt` 标注，并把 reset 行为和原 HelmRelease condition 写入报告。普通 Progressing 状态只触发常规 reconcile，不打 reset。若 Flux、Helm、目标 image tag、rollout 或 migration 任一步失败，release gate 会采集 HelmRelease JSON、namespace events、API/Admin Pod 日志摘要后停止，不继续运行 health 或 quick live eval，避免把旧版本健康误报为目标版本可用。
...
-> runtime Secret contract passed
-> Flux sync healthy
...
| K8s rollout / health / live eval 失败 | 通知 release owner，并附 runtime Secret contract、HelmRelease conditions、namespace events、Pod 日志摘要、rollout 状态、health 响应、eval 报告路径和回滚建议。 |
