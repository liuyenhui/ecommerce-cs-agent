...
System Admin 所有面向用户的日期时间固定按 `Asia/Shanghai` 显示，中文格式为 `YYYY年M月D日 HH:mm:ss`，不依赖浏览器或操作系统时区；空值和非法值统一显示 `—`。API 查询参数、响应、数据库字段、原始技术 JSON 与审计差异仍保持 UTC/RFC3339/ISO 契约，`datetime-local` 输入提交时继续转换为 UTC ISO 值。
