...
- 系统后台 UI 的源文档是本文，视觉参照是 [System Admin UI Prototype](system-admin-ui-prototype.html)。
- 客户后台 UI 的源文档是 [Customer Admin Design](customer-admin-design.md)。系统后台文档不重复客户后台页面细节，只保留两个后台的隔离边界。
- README、AGENTS、架构摘要和开发交接只作为入口和维护规则，不复制完整 UI 规范。
- 如果后续调整系统后台布局、导航分组、视觉密度、状态表达、抽屉、模态、筛选或表格规则，应优先更新本文和原型；只有当架构摘要提到这些规则时，再同步对应摘要。

### 5.0 当前实现基准

当前 Admin Web 第一版实现应与原型保持以下可验证基准：

- 独立入口：系统后台按 `system-admin.ecommerce-cs-agent-dev.fcihome.com` 或本地 `/system-admin` 进入；不得通过客户后台 shell 内的 tab、workspace switch 或按钮切换进入。
- 专用登录：系统后台登录页是系统后台专用 `/login`，成功后只建立 `agent_system_admin_session` 或等价系统后台 session。
- 黑色顶栏：顶栏包含产品标识、租户范围选择、全局搜索、告警 / 刷新入口和当前系统用户。
- 浅灰分组导航：左侧导航按“平台运营 / 排障治理 / 发布安全”分组，必须包含系统首页、租户与店铺、配置完成度、资料与知识、决策追踪、异步任务、规则与动作、评测与发布、安全审计、系统健康。
- 三栏工作台：桌面端使用左侧导航、主内容区、右侧运行摘要三栏；右侧上下文栏展示租户范围、上线阻断、运行任务、健康异常和高优先级告警。
- 页面结构：主内容区统一使用英文小标签、中文标题、简短说明、主操作、指标、筛选、表格 / 列表或流程区。
- 交互基线：导航高亮、筛选、全局搜索、详情抽屉、创建 / 发布 / 冻结等模态、Toast 和危险操作二次确认都必须可验证。
- 表格基线：表头浅灰、行分隔 1px、关键 ID 使用等宽字体；移动端通过关键列或 `data-label` 列表降级。
- 安全基线：系统后台不能伪装客户用户调用 `/v1/admin/*`；代客户操作必须走系统后台专用接口并写系统审计。
