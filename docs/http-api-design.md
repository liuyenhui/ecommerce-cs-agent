...
- 客户后台登录默认只提交邮箱和密码；外部入口优先通过 open_erp_agent 微信授权桥接进入，Customer Admin 登录页按钮跳转到 `https://www.fcihome.com/ai-cs/customer-admin-login`。open_erp_agent 只在自身微信 web session 和店铺管理权限校验通过后签发一次性 launch ticket，Customer Admin 兑换后只建立客户 Admin session，例如 `agent_admin_session`，并跳转 `/admin`；组织 / 店铺上下文来自 `GET /v1/admin/auth/me` 返回的可访问列表，客户后台路由守卫只调用该接口。
