...
const customerStyles = read('admin-web/customer-admin/src/styles.css');
const styles = [
...
const checks = [
  ...
  ['Customer message history renders localized decision badges without visible raw enums', customerApp.includes('presentDecisionBadges') && messageHistory.includes('decisionBadges(trace)') && customerApp.includes('className="decisionBadges"') && customerApp.includes('className={`decisionBadge ${badge.tone}`}') && customerApp.includes('title={badge.raw}') && !customerApp.includes('`动作 ${trace.action}`') && !customerApp.includes('`风险 ${trace.risk_level}`')],
  ['Customer decision badges use compact semantic Carbon-style pills', customerStyles.includes('.decisionBadges') && customerStyles.includes('.decisionBadge.info') && customerStyles.includes('.decisionBadge.success') && customerStyles.includes('.decisionBadge.warning') && customerStyles.includes('.decisionBadge.danger') && customerStyles.includes('.decisionBadge.neutral')],
  ...
  ['Decision replay exposes an X6 failure fallback while retaining node navigation', sharedTraceReplay.includes('流程图暂时无法显示，请查看下方节点导航。') && sharedTraceReplay.includes('dispatchGraphUi({ type: "failed" })') && sharedTraceReplay.includes('aria-label="处理步骤"')],
  ...
  ['Decision graph failure hides the canvas and offers retry before node navigation', sharedTraceReplay.includes('hidden={graphUnavailable}') && sharedTraceReplay.includes('重试流程图') && sharedTraceReplay.includes('retryKey') && sharedTraceReplay.indexOf('重试流程图') < sharedTraceReplay.indexOf('aria-label="处理步骤"')],
  ['Decision replay uses compact localized trace node badges with progress', sharedTraceReplay.includes('traceNodeNavigationHeader') && sharedTraceReplay.includes('<strong>处理步骤</strong>') && sharedTraceReplay.includes('{progress.label}') && sharedTraceReplay.includes('className="traceNodeBadges"') && sharedTraceReplay.includes('aria-pressed={selected}') && sharedTraceReplay.includes('presentTraceNode') && sharedTraceReplay.includes('traceNodeState') && sharedTraceReplay.includes('aria-hidden="true"')],
  ['Trace node badges preserve empty raw status for presentation and diagnostics', sharedTraceReplay.includes('presentTraceNode(node.status, isCurrent)') && sharedTraceReplay.includes('title={`${node.id} · ${nodePresentation.raw}`}')],
  ['Current trace node badge uses an independent class and blue left-side emphasis line', sharedTraceReplay.includes('${isCurrent ? " is-current" : ""}') && styles.includes('.traceNodeBadge.is-current') && styles.includes('box-shadow: inset 3px 0 #2563eb')],
  ['Failed current trace nodes retain failure styling', styles.includes('.traceNodeBadge.failed') && styles.includes('.traceNodeBadge.failed.is-current')],
  ['Trace node badges use an opaque focus indicator with clipping clearance', styles.includes('outline: 3px solid #0f62fe') && styles.includes('outline-offset: 0') && styles.includes('.traceNodeBadges') && styles.includes('padding: 3px')],
  ['Decision replay no longer uses the legacy trace timeline cards', !sharedTraceReplay.includes('traceTimeline') && !styles.includes('.traceTimeline') && !styles.includes('min-height: 58px')],
