...
export function TracesPage({ state, detail, onSearch, onOpen, onClose, onPageChange }: {
  ...
  return <>
    ...
    <RequestStateView state={state}>{(data) => <><p className="pageTotal">共 {data.page.total} 条决策</p><DataTable title="消息决策" rows={data.items} fields={["decision_id", "organization_id", "store_id", "action", "status", "created_at"]} action={(row) => <button type="button" onClick={() => onOpen(String(row.decision_id || ""))}>运行回放</button>} emptyState={{ title: "暂无决策记录", description: "请提供租户、店铺、Decision ID 或时间范围查询真实记录。" }} /><PaginationControls page={data.page} onPageChange={onPageChange || (() => undefined)} /></>}</RequestStateView>
