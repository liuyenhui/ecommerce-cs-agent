...
export function LlmGovernancePage({ roles = ["super_admin"] }: { roles?: string[] }) {
  ...
  function changeOrganization(value: string) {
    ...
    setVersions([]); setVersionPage({ has_more: false, next_cursor: null }); setSelectedId(""); setRoutes([]); setDirty(false); setOrganizationId(value);
  }
  ...
  return <section className="llmPage">
    ...
    {tab === "config" ? <ConfigurationTab canWrite={canWrite} canTest={canTest} providers={providers} organizationId={organizationId} setOrganizationId={changeOrganization} loadVersions={loadVersions} busy={busy} selected={selected} routes={routes} patchRoute={patchRoute} dirty={dirty} saveRoutes={saveRoutes} validateDraft={validateDraft} connectionTest={connectionTest} providerEditor={providerEditor} setProviderEditor={setProviderEditor} createProvider={createProvider} editingProvider={editingProvider} setEditingProvider={setEditingProvider} updateProvider={updateProvider} draftEditor={draftEditor} setDraftEditor={setDraftEditor} createDraft={createDraft} /> : null}
    {tab === "usage" ? <UsageTab usageState={usageState} points={points} breakdown={breakdown} scenarioBreakdown={scenarioBreakdown} failureBreakdown={failureBreakdown} invocations={invocations} filters={filters} onFilterChange={changeUsageFilter} onLoad={loadUsage} onLoadMore={loadMoreInvocations} hasMore={usageState.kind === "success" && invocationPage.has_more} loadMoreBusy={invocationMoreBusy} loading={busy === "usage"} /> : null}
    {tab === "versions" ? <VersionsTab organizationId={organizationId} setOrganizationId={changeOrganization} loadVersions={loadVersions} loadMore={loadMoreVersions} hasMore={versionPage.has_more} loadMoreBusy={versionMoreBusy} versions={versions} loading={busy === "versions"} /> : null}
...
function ConfigurationTab(props: { canWrite: boolean; canTest: boolean; providers: LlmProvider[]; organizationId: string; setOrganizationId: (v: string) => void; loadVersions: () => void; busy: string; selected?: LlmVersion; routes: LlmRoute[]; patchRoute: (i: number, patch: Partial<LlmRoute>) => void; dirty: boolean; saveRoutes: () => void; validateDraft: () => void; connectionTest: (provider: LlmProvider) => void; providerEditor: boolean; setProviderEditor: (open: boolean) => void; createProvider: (body: Record<string, unknown>) => void; editingProvider: LlmProvider | null; setEditingProvider: (provider: LlmProvider | null) => void; updateProvider: (provider: LlmProvider, body: Record<string, unknown>) => void; draftEditor: boolean; setDraftEditor: (open: boolean) => void; createDraft: (body: Record<string, unknown>) => void }) {
  return <div role="tabpanel" className="llmConfigGrid">
    ...
    <section className="llmPanel"><h2>场景模型路由</h2><p className="panelHelp">主模型与可选降级模型按业务场景独立配置。</p>
      {props.routes.length ? props.routes.map((route, index) => <div className="routeRow" key={route.scenario}><strong>{route.scenario}</strong><label>主模型<input className="modelId" value={route.primary_model} onChange={(e) => props.patchRoute(index, { primary_model: e.target.value })} disabled={props.selected?.status !== "draft"} /></label><label>降级模型<input className="modelId" value={route.fallback_model || ""} onChange={(e) => props.patchRoute(index, { fallback_model: e.target.value || null })} disabled={props.selected?.status !== "draft"} /></label></div>) : <p className="structuredEmpty">加载组织后显示服务端配置的场景路由。</p>}
    </section>
...
function ProviderEditor({ busy, onCancel, onSave }: { busy: boolean; onCancel: () => void; onSave: (body: Record<string, unknown>) => void }) {
  ...
  return <div className="providerEditor" role="group" aria-label="新增 Provider 表单"><label>Provider 名称<input value={form.name} onChange={(e) => set("name", e.target.value)} /></label><label>Provider 类型<select value={form.provider_type} onChange={(e) => set("provider_type", e.target.value)}><option value="openai">OpenAI</option><option value="openai_compatible">OpenAI Compatible</option><option value="anthropic">Anthropic</option><option value="azure_openai">Azure OpenAI</option></select></label><label>Base URL<input value={form.base_url} onChange={(e) => set("base_url", e.target.value)} placeholder="https://" /></label><label>Secret namespace<input className="secretRef" value={form.namespace} onChange={(e) => set("namespace", e.target.value)} /></label><label>Secret name<input className="secretRef" value={form.secret_name} onChange={(e) => set("secret_name", e.target.value)} /></label><label>Secret key<input className="secretRef" value={form.secret_key} onChange={(e) => set("secret_key", e.target.value)} /></label><label>变更原因<input value={form.reason} onChange={(e) => set("reason", e.target.value)} /></label><label>创建幂等键<input value={form.idempotency_key} onChange={(e) => set("idempotency_key", e.target.value)} /></label><div className="panelActions"><button onClick={onCancel}>取消</button><button className="primaryAction" disabled={!valid || busy} onClick={() => onSave({ name: form.name.trim(), provider_type: form.provider_type, base_url: form.base_url.trim(), secret_ref: { namespace: form.namespace.trim(), name: form.secret_name.trim(), key: form.secret_key.trim() }, enabled: true, reason: form.reason.trim(), idempotency_key: form.idempotency_key.trim() })}>保存 Provider</button></div></div>;
}
...
function UsageTab({ usageState, points, breakdown, scenarioBreakdown, failureBreakdown, invocations, filters, onFilterChange, onLoad, onLoadMore, hasMore, loadMoreBusy, loading }: { usageState: UsageState; points: LlmUsagePoint[]; breakdown: LlmBreakdown[]; scenarioBreakdown: LlmBreakdown[]; failureBreakdown: LlmBreakdown[]; invocations: LlmInvocation[]; filters: LlmUsageFilters; onFilterChange: (key: keyof LlmUsageFilters, value: string | undefined) => void; onLoad: () => void; onLoadMore: () => void; hasMore: boolean; loadMoreBusy: boolean; loading: boolean }) {
  ...
  return <div role="tabpanel" className="usageWorkspace"><div className="usageFilters"><label>开始时间<input type="datetime-local" onChange={(e) => update("start_at", e.target.value ? new Date(e.target.value).toISOString() : "")} /></label><label>结束时间<input type="datetime-local" onChange={(e) => update("end_at", e.target.value ? new Date(e.target.value).toISOString() : "")} /></label><label>Provider ID<input onChange={(e) => update("provider_config_id", e.target.value)} /></label><label>模型<input className="modelId" onChange={(e) => update("model", e.target.value)} /></label><label>场景<input onChange={(e) => update("scenario", e.target.value)} /></label><label>组织<input onChange={(e) => update("organization_id", e.target.value)} /></label><label>店铺<input onChange={(e) => update("store_id", e.target.value)} /></label><button onClick={onLoad}>{loading ? "重新查询" : "查询用量"}</button></div>
    ...
    {usageState.kind === "success" ? <>{points.length ? <section className="llmPanel" data-testid="usage-chart"><h2>用量时序</h2><div className="timeseriesBars">{points.map((point) => <div key={`${point.bucket}-${point.currency}`}><span>{new Date(point.bucket).toLocaleString()}</span><meter min="0" max={Math.max(...points.map((p) => p.calls), 1)} value={point.calls} /><strong>{point.calls} 次</strong></div>)}</div></section> : null}
    <section className="llmPanel"><h2>模型成本分布</h2>{breakdown.length ? <div className="tableScroll modelCostTable"><table><thead><tr><th>模型</th><th>调用</th><th>Token</th><th>估算成本</th></tr></thead><tbody>{breakdown.map((item) => <tr key={`${item.key}-${item.currency}`}><td><span className="modelId">{item.key}</span></td><td>{item.calls}</td><td>{item.total_tokens}</td><td>{money(item.estimated_cost_micros, item.currency)}</td></tr>)}</tbody></table></div> : <p className="structuredEmpty">暂无模型或场景分布。</p>}</section>
    <BreakdownPanel title="场景用量分布" label="场景" items={scenarioBreakdown} />
    <BreakdownPanel title="失败原因分布" label="错误码" items={failureBreakdown} />
    <section className="llmPanel"><h2>调用明细</h2>{invocations.length ? <div className="tableScroll"><table><thead><tr><th>调用 / 时间</th><th>Provider</th><th>组织 / 店铺</th><th>模型 / 场景</th><th>路由 / 延迟</th><th>Token / 成本</th><th>状态 / 失败原因</th></tr></thead><tbody>{invocations.map((item) => <tr key={item.invocation_id}><td><span>{item.invocation_id}</span><br />{new Date(item.occurred_at).toLocaleString()}</td><td><span>{item.provider_name}</span><br /><span>{item.provider_config_id}</span></td><td>{item.organization_id}<br />{item.store_id || "—"}</td><td><span className="modelId">{item.model}</span><br /><span>{item.scenario}</span></td><td><span>{item.route_role}</span><br /><span>调用延迟：{item.latency_ms} ms</span></td><td><span>输入 {item.input_tokens}</span><br /><span>输出 {item.output_tokens}</span><br /><span>成本：{money(item.estimated_cost_micros, item.currency)}</span></td><td>{item.status}<br /><span>{item.error_code || "—"}</span></td></tr>)}</tbody></table></div> : <p className="structuredEmpty">暂无脱敏调用明细。</p>}{hasMore ? <button disabled={loadMoreBusy} onClick={onLoadMore}>{loadMoreBusy ? "加载中…" : "加载更多调用"}</button> : null}</section></> : null}
  </div>;
}

function BreakdownPanel({ title, label, items }: { title: string; label: string; items: LlmBreakdown[] }) { return <section className="llmPanel"><h2>{title}</h2>{items.length ? <div className="tableScroll"><table><thead><tr><th>{label}</th><th>调用</th><th>Token</th><th>估算成本</th></tr></thead><tbody>{items.map((item) => <tr key={`${item.key}-${item.currency}`}><td><span>{item.key || "未标记"}</span></td><td>{item.calls}</td><td>{item.total_tokens}</td><td>{money(item.estimated_cost_micros, item.currency)}</td></tr>)}</tbody></table></div> : <p className="structuredEmpty">当前筛选范围内暂无{title}。</p>}</section>; }
