...
function withTimezoneAwareBounds(filters: AuditFilters): AuditFilters {
  return {
    ...filters,
    time_from: filters.time_from ? new Date(filters.time_from).toISOString() : "",
    time_to: filters.time_to ? new Date(filters.time_to).toISOString() : ""
  };
}

export function AuditPage({ state, onSearch, onPageChange = () => undefined }: { state: RequestState<PageEnvelope>; onSearch: (filters: AuditFilters) => void; onPageChange?: (page: number) => void }) {
  ...
  return <>
    ...
    <section className="auditFilters">
      ...
      <button type="button" onClick={() => onSearch(withTimezoneAwareBounds(filters))}><Search size={16} />查询</button>
    </section>
    <RequestStateView state={state}>{(data) => <><DataTable title={`审计记录（共 ${data.page.total} 条）`} rows={data.items} fields={["audit_log_id", "actor_system_user_id", "organization_id", "store_id", "action", "sensitive_access", "created_at"]} emptyState={{ title: "暂无审计记录", description: "服务端未返回符合筛选条件的审计记录。" }} /><PaginationControls page={data.page} onPageChange={onPageChange} /></>}</RequestStateView>
  </>;
}
