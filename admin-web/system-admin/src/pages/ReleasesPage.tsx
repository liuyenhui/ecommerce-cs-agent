...
export function ReleasesPage({ roles = ["super_admin"] }: { roles?: string[] }) {
  ...
  const controller = React.useRef<AbortController | null>(null); const versionMoreRequest = React.useRef<AbortController | null>(null); const releaseMoreRequest = React.useRef<AbortController | null>(null); const generation = React.useRef(0);
  const versionMoreInFlight = React.useRef(false); const releaseMoreInFlight = React.useRef(false);
  const [versionMoreBusy, setVersionMoreBusy] = React.useState(false); const [releaseMoreBusy, setReleaseMoreBusy] = React.useState(false); const [lookupBusy, setLookupBusy] = React.useState("");
  const dialogRef = React.useRef<HTMLDivElement | null>(null);
  const restoreFocus = React.useRef<HTMLElement | null>(null);
  React.useEffect(() => () => { controller.current?.abort(); versionMoreRequest.current?.abort(); releaseMoreRequest.current?.abort(); }, []);

  function changeOrganization(value: string) {
    controller.current?.abort(); versionMoreRequest.current?.abort(); releaseMoreRequest.current?.abort(); generation.current += 1;
    versionMoreInFlight.current = false; releaseMoreInFlight.current = false; setVersionMoreBusy(false); setReleaseMoreBusy(false);
    setVersions([]); setRecords([]); setVersionCursor(null); setReleaseCursor(null); setSelected(null); setOrganizationId(value);
  }
  ...
  async function load(preserve = false): Promise<boolean> {
    ...
    controller.current?.abort(); versionMoreRequest.current?.abort(); releaseMoreRequest.current?.abort(); versionMoreInFlight.current = false; releaseMoreInFlight.current = false; setVersionMoreBusy(false); setReleaseMoreBusy(false); const next = new AbortController(); controller.current = next; const current = ++generation.current; const scope = organizationId.trim();
  ...
  async function loadMoreVersions() { if (!versionCursor || versionMoreInFlight.current) return; versionMoreInFlight.current = true; const current = generation.current; const scope = organizationId.trim(); const cursor = versionCursor; const next = new AbortController(); versionMoreRequest.current = next; setVersionMoreBusy(true); try { const data = await systemApi.llmVersions(scope, cursor, next.signal); if (next.signal.aborted || current !== generation.current || scope !== organizationId.trim()) return; setVersions((items) => [...items, ...data.items]); setVersionCursor(data.page_info.next_cursor); } finally { versionMoreInFlight.current = false; if (current === generation.current) setVersionMoreBusy(false); } }
  async function loadMoreRecords() { if (!releaseCursor || releaseMoreInFlight.current) return; releaseMoreInFlight.current = true; const current = generation.current; const scope = organizationId.trim(); const cursor = releaseCursor; const next = new AbortController(); releaseMoreRequest.current = next; setReleaseMoreBusy(true); try { const data = await systemApi.llmReleaseRecords(scope, cursor, next.signal); if (next.signal.aborted || current !== generation.current || scope !== organizationId.trim()) return; setRecords((items) => [...items, ...data.items]); setReleaseCursor(data.page_info.next_cursor); } finally { releaseMoreInFlight.current = false; if (current === generation.current) setReleaseMoreBusy(false); } }
  async function loadVersionForRecord(versionId: string) { setLookupBusy(versionId); try { const item = await systemApi.llmVersion(versionId); if (item.organization_id !== organizationId.trim()) throw new Error("版本组织范围不匹配"); setVersions((items) => items.some((version) => version.version_id === item.version_id) ? items : [...items, item]); setMessage("已加载发布记录对应版本，可执行回滚"); } catch (error) { setMessage(`对应版本加载失败：${failure(error)}`); } finally { setLookupBusy(""); } }
  ...
  async function publish() {
    ...
    try { await systemApi.publishLlmVersion(selected.version_id, { expected_revision: selected.revision, reason: reason.trim(), idempotency_key: key.trim() }); setSelected(null); const refreshed = await load(true); setMessage(refreshed ? "版本已发布，列表已从服务端刷新" : "发布已成功，但列表刷新失败，请重试"); }
  ...
  async function submit() {
    ...
    try { await systemApi.submitLlmVersion(selected.version_id, { expected_revision: selected.revision, evaluation_run_id: evaluationId.trim(), reason: reason.trim(), idempotency_key: key.trim() }); setSelected(null); const refreshed = await load(true); setMessage(refreshed ? "版本已绑定评测快照，列表已从服务端刷新" : "提交已成功，但列表刷新失败，请重试"); }
    catch (error) { setMessage(failure(error)); }
  }
  async function rollback(version: LlmVersion) {
    ...
    try { await systemApi.rollbackLlmVersion(version.version_id, { reason: rollbackReason, idempotency_key: `rollback-${crypto.randomUUID()}` }); const refreshed = await load(true); setMessage(refreshed ? "回滚已完成，版本与发布记录已从服务端刷新" : "回滚已成功，但列表刷新失败，请重试"); }
    catch (error) { setMessage(failure(error)); }
  }
  ...
  return <section className="llmPage"><header className="pageHeading"><div><h1>评测与发布</h1><p>发布只使用真实版本记录及其已绑定评测快照；当前契约不提供独立评测运行列表。</p></div></header>
    <div className="organizationLoader"><label>组织 ID<input value={organizationId} onChange={(event) => changeOrganization(event.target.value)} /></label><button onClick={() => void load(false)}>查询版本</button></div>
